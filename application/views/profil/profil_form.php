...
$(document).ready(function() {
    ...
    $('#btn-filter').click(function(){ //button filter event click
        reload_profil_table();  //just reload table
    });
...
function edit_profil(id)
{
    ...
    $.ajax({
        ...
        {
			$('[name="id"]').val(data.id_profil);
			$('[name="judul_profil"]').val(data.judul_profil);
			$('[name="isi_profil"]').val(data.isi_profil);
			...
			if(data.photo)
            {
                ...
                $('#photo-preview div').html('<img src="'+base_url+'upload/profil/thumbs/'+data.photo+'" class="img-responsive">'); // show photo
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "<?php echo site_url('profil/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
        ...
        <div class="modal-content">
            ...
            <div class="modal-body form">
				...
				<div class="form-group">
					<label class="control-label col-md-3" for="judul_profil">Judul Profil</label>
					<div class="col-md-9">
    				<input placeholder="Judul Profil" type="text" name="judul_profil" class="form-control" />		
					<span class="help-block"></span>
					</div>
				</div>
				
				<div class="form-group">
					<label class="control-label col-md-3" for="isi_profil">Isi Profil</label>
					<div class="col-md-9">
					<textarea class="form-control" name="isi_profil" id="isi_profil"><?php echo set_value('isi_profil', isset($default['isi_profil']) ? $default['isi_profil'] : ''); ?></textarea>
					<span class="help-block"></span>
					</div>
				</div>
